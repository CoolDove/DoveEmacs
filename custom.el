(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(ansi-color-faces-vector
   [default default default italic underline success warning error])
 '(ansi-color-names-vector
   ["black" "red3" "ForestGreen" "yellow3" "blue" "magenta3" "DeepSkyBlue" "gray50"])
 '(awesome-tray-mode-line-active-color "#0031a9")
 '(awesome-tray-mode-line-inactive-color "#d7d7d7")
 '(blink-cursor-mode nil)
 '(compilation-message-face 'default)
 '(custom-safe-themes
   '("7b8f5bbdc7c316ee62f271acf6bcd0e0b8a272fdffe908f8c920b0ba34871d98" "d445c7b530713eac282ecdeea07a8fa59692c83045bf84dd112dd738c7bcad1d" "4c56af497ddf0e30f65a7232a8ee21b3d62a8c332c6b268c81e9ea99b11da0d3" "aca70b555c57572be1b4e4cec57bc0445dcb24920b12fb1fea5f6baa7f2cad02" "1ca05bdae217adeb636e9bc5e84c8f1d045be2c8004fafd5337d141d9b67a96f" "bd3b9675010d472170c5d540dded5c3d37d83b7c5414462737b60f44351fb3ed" "830c887bd2cefd77326ddf24389f3806c2a561a550e309691933f772f1bc5825" "3c83b3676d796422704082049fc38b6966bcad960f896669dfc21a7a37a748fa" "e76668f7efa2c9297c3998aba61876e757c99c1e5325c135433cf4f53a95df15" "1a1266e25ed97448bbe80f246f53372d4b914d30802711abfda7afcbf2f7b3ec" "c560237b7505f67a271def31c706151afd7aa6eba9f69af77ec05bde5408dbcd" "5111a41453244802afd93eed1a434e612a8afbdf19c52384dffab129258bab6e" "3f3c48d3835286245137ad2fffbe43c634fef7f33500b008ec3cecc3672e7e3b" "6df412e59dbfe7f72f24319b9ee4513e40bb0e44384fc93a2c77399e641348f6" "69b30fcd01e0bce8accefc2fd2f241b84ecbec13ec49719cdda5df550073886e" "36331aa9a054f3e8a2b7e3a1139287e3159596b2ad4166e8e0d47928282f8e77" "e01db763cd9daa56f75df8ebd057f84017ae8b5f351ec90c96c928ad50f3eb25" "28eb6d962d45df4b2cf8d861a4b5610e5dece44972e61d0604c44c4aad1e8a9d" "143d897548e5a7efb5cf92c35bd39fe7c90cbd28f9236225ad3e80e1b79cef8a" "0754c176c3f850e1f90e177e283247749a1ac688faebf05b6016343cb3f00064" "0710b0bdd59c8a7aacf0640591b38fcad5978a0fcfff3fdd999e63499ada8e3e" "37768a79b479684b0756dec7c0fc7652082910c37d8863c35b702db3f16000f8" "0cd00c17f9c1f408343ac77237efca1e4e335b84406e05221126a6ee7da28971" "c74e83f8aa4c78a121b52146eadb792c9facc5b1f02c917e3dbb454fca931223" "cf9414f229f6df728eb2a5a9420d760673cca404fee9910551caf9c91cff3bfa" "dbade2e946597b9cda3e61978b5fcc14fa3afa2d3c4391d477bdaeff8f5638c5" "801a567c87755fe65d0484cb2bded31a4c5bb24fd1fe0ed11e6c02254017acb2" "1df2d767cc1b5ed78626f93f06c24ac15144a28b7420364769bf63cd23e420d3" "6bdcff29f32f85a2d99f48377d6bfa362768e86189656f63adbf715ac5c1340b" "a27c00821ccfd5a78b01e4f35dc056706dd9ede09a8b90c6955ae6a390eb1c1e" "6b5c518d1c250a8ce17463b7e435e9e20faa84f3f7defba8b579d4f5925f60c1" "7661b762556018a44a29477b84757994d8386d6edee909409fabe0631952dad9" "4eb6fa2ee436e943b168a0cd8eab11afc0752aebb5d974bba2b2ddc8910fca8f" "83e0376b5df8d6a3fbdfffb9fb0e8cf41a11799d9471293a810deb7586c131e6" default))
 '(display-line-numbers-type 'relative t)
 '(display-time-mode t)
 '(fci-rule-color "#FFFFF8")
 '(flymake-error-bitmap '(flymake-double-exclamation-mark modus-theme-fringe-red))
 '(flymake-note-bitmap '(exclamation-mark modus-theme-fringe-cyan))
 '(flymake-warning-bitmap '(exclamation-mark modus-theme-fringe-yellow))
 '(highlight-changes-colors '("#ff8eff" "#ab7eff"))
 '(highlight-tail-colors '(("#aecf90" . 0) ("#c0efff" . 20)))
 '(hl-sexp-background-color "#efebe9")
 '(hl-todo-keyword-faces
   '(("HOLD" . "#70480f")
	 ("TODO" . "#721045")
	 ("NEXT" . "#5317ac")
	 ("THEM" . "#8f0075")
	 ("PROG" . "#00538b")
	 ("OKAY" . "#30517f")
	 ("DONT" . "#315b00")
	 ("FAIL" . "#a60000")
	 ("BUG" . "#a60000")
	 ("DONE" . "#005e00")
	 ("NOTE" . "#863927")
	 ("KLUDGE" . "#813e00")
	 ("HACK" . "#813e00")
	 ("TEMP" . "#5f0000")
	 ("FIXME" . "#a0132f")
	 ("XXX+" . "#972500")
	 ("REVIEW" . "#005a5f")
	 ("DEPRECATED" . "#201f55")))
 '(ibuffer-deletion-face 'modus-theme-mark-del)
 '(ibuffer-filter-group-name-face 'modus-theme-mark-symbol)
 '(ibuffer-marked-face 'modus-theme-mark-sel)
 '(ibuffer-title-face 'modus-theme-pseudo-header)
 '(indicate-buffer-boundaries '((t . right) (top . left)))
 '(line-number-mode nil)
 '(magit-diff-use-overlays nil)
 '(org-agenda-files
   '("~/.emacs.d/dorg/dove.org" "~/.emacs.d/dorg/cocowork.org" "~/.emacs.d/dorg/dove.org" "~/.emacs.d/dorg/cocowork.org"))
 '(package-selected-packages
   '(yaml-mode dired-git gdscript-mode zig-mode lsp-java ivy-xref evil-goggles lsp-ivy ivy-lsp evil-snipe markdown-mode obsidian-theme python-mode solarized-theme nhexl-mode cape lsp-mode cosmo railscasts-theme jazz-theme visible-mark back-button devdocs-browser markdown-preview-eww w3m popup-kill-ring kill-ring markdown-toc csharp-mode org-bullets heaven-and-hell darkroom pdf-tools dim eldoc-box dimmer diminish auto-dim-other-buffers mosey find-file-rg web-search quick-peek zoom-window on-screen elscreen centaur-tabs magit evil-ex-shell-command goto-last-change undo-tree counsel shader-mode hlsl-mode glsl-mode deadgrep evil-mc evil-anzu dove-linum pyim youdao-dictionary use-package smooth-scroll smart-mode-line restart-emacs projectile indent-guide gruvbox-theme evil-surround company all-the-icons))
 '(pos-tip-background-color "#E6DB74")
 '(pos-tip-foreground-color "#242728")
 '(safe-local-variable-values '((eval when (fboundp 'rainbow-mode) (rainbow-mode 1))))
 '(scroll-bar-mode nil)
 '(tool-bar-mode nil)
 '(tooltip-mode nil)
 '(vc-annotate-background "#FFFFFD")
 '(vc-annotate-background-mode nil)
 '(vc-annotate-color-map
   '((20 . "#D5D2C8")
	 (40 . "#B0ADA2")
	 (60 . "#B0ADA2")
	 (80 . "#737063")
	 (100 . "#737063")
	 (120 . "#4E4B3D")
	 (140 . "#4E4B3D")
	 (160 . "#373426")
	 (180 . "#373426")
	 (200 . "#373426")
	 (220 . "#292617")
	 (240 . "#292617")
	 (260 . "#292617")
	 (280 . "#201D0E")
	 (300 . "#201D0E")
	 (320 . "#201D0E")
	 (340 . "#1B1809")
	 (360 . "#1B1809")))
 '(vc-annotate-very-old-color "#4E4B3D")
 '(weechat-color-list
   (unspecified "#242728" "#323342" "#F70057" "#ff0066" "#86C30D" "#63de5d" "#BEB244" "#E6DB74" "#40CAE4" "#06d8ff" "#FF61FF" "#ff8eff" "#00b2ac" "#53f2dc" "#f8fbfc" "#ffffff"))
 '(window-divider-default-places 'bottom-only)
 '(xterm-color-names
   ["#000000" "#a60000" "#005e00" "#813e00" "#0031a9" "#721045" "#00538b" "#f0f0f0"])
 '(xterm-color-names-bright
   ["#505050" "#972500" "#315b00" "#70480f" "#2544bb" "#8f0075" "#30517f" "#ffffff"]))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(default ((t (:foreground "#d3b58d" :background "#041818" :family "Victor Mono" :foundry "outline" :slant normal :weight normal :height 120 :width normal))))
 '(bold ((t (:weight normal :family "unifont"))))
 '(custom-group-tag-face ((t (:underline t :foreground "lightblue"))) t)
 '(custom-variable-tag-face ((t (:underline t :foreground "lightblue"))) t)
 '(font-lock-builtin-face ((t nil)))
 '(font-lock-comment-face ((t (:foreground "#3fdflf"))))
 '(font-lock-function-name-face ((((class color) (background dark)) (:foreground "white"))))
 '(font-lock-keyword-face ((t (:foreground "white"))))
 '(font-lock-string-face ((t (:foreground "#0fdfaf"))))
 '(font-lock-variable-name-face ((((class color) (background dark)) (:foreground "#c8d4ec"))))
 '(font-lock-warning-face ((t (:foreground "#504038"))))
 '(highlight ((t (:foreground "navyblue" :background "darkseagreen2"))))
 '(mode-line ((t (:inverse-video t))))
 '(mode-line-emphasis ((t (:family "unifont"))))
 '(region ((t (:background "blue"))))
 '(widget-field-face ((t (:foreground "white"))) t)
 '(widget-single-line-field-face ((t (:background "darkgray"))) t))
